...
void Footer::EncodeTo(std::string *dst) const
{
	...
	if (IsLegacyFooterFormat(table_magic_number())) {
		...
		PutFixed32(dst, static_cast<uint32_t>(table_magic_number() &
						      0xffffffffu));
		PutFixed32(dst,
		...
		PutFixed32(dst,
			   static_cast<uint32_t>(table_magic_number() >> 32));
		assert(dst->size() ==
		       original_size + kNewVersionsEncodedLength);
	}
}

Footer::Footer(uint64_t _table_magic_number, uint32_t _version)
	: version_(_version), checksum_(kCRC32c),
	...
	if (legacy) {
		...
		if (input->size() < kNewVersionsEncodedLength) {
			return Status::Corruption(
				"input is too short to be an sstable");
		} else {
...
std::string Footer::ToString() const
{
	...
	if (legacy) {
		result.append("metaindex handle: " +
			      metaindex_handle_.ToString() + "\n  ");
			  ...
			  Footer *footer, uint64_t enforce_table_magic_number)
{
	...
	size_t read_offset =
		(file_size > Footer::kMaxEncodedLength) ?
			      ...
			      0;
	Status s = file->Read(read_offset, Footer::kMaxEncodedLength,
		 ...
		 Slice *contents, /* result of reading */ char *buf)
{
...

	{
		...
		s = file->Read(handle.offset(), n + kBlockTrailerSize, contents,
			       buf);
	}
	...
	if (options.verify_checksums) {
		...
		case kxxHash:
			actual =
				XXH32(data, static_cast<int>(n) + 1, 0);
			break;
		default:
			 ...
			 const PersistentCacheOptions &cache_options)
{
	...
	if (status.ok()) {
		...
		if (decompression_requested &&
			...
			heap_buf = std::unique_ptr<char[]>(
				new char[n + kBlockTrailerSize]);
			used_buf = heap_buf.get();
		}

		status = ReadBlock(file, footer, read_options, handle, &slice,
				   used_buf);
		if (status.ok() && read_options.fill_cache &&
		    cache_options.persistent_cache &&
		    cache_options.persistent_cache->IsCompressed()) {
			// insert to raw cache
			PersistentCacheHelper::InsertRawPage(
				cache_options, handle, used_buf,
				n + kBlockTrailerSize);
		}
	}
	...
	if (decompression_requested && compression_type != kNoCompression) {
		...
		*contents = BlockContents(Slice(slice.data(), n), false,
					  compression_type);
	} else {
		...
		*contents = BlockContents(std::move(heap_buf), n, true,
					  compression_type);
	}
	...
	CompressionType compression_type, const ImmutableCFOptions &ioptions)
{
	...
	StopWatchNano timer(ioptions.env,
			    ShouldReportDetailedTime(ioptions.env,
						     ioptions.statistics));
	...
	case kSnappyCompression: {
		...
		*contents = BlockContents(std::move(ubuf), ulength, true,
					  kNoCompression);
		break;
	}
	...
	case kLZ4Compression:
		ubuf.reset(
			LZ4_Uncompress(data, n, &decompress_size,
				       GetCompressFormatForVersion(
					       kLZ4Compression, format_version),
				       compression_dict));
		if (!ubuf) {
	...
	if (ShouldReportDetailedTime(ioptions.env, ioptions.statistics)) {
		MeasureTime(ioptions.statistics, DECOMPRESSION_TIMES_NANOS,
			    timer.ElapsedNanos());
		MeasureTime(ioptions.statistics, BYTES_DECOMPRESSED,
			    contents->data.size());
