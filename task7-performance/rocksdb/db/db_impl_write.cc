			 ...
			 bool disable_memtable)
{
	...
	if (immutable_db_options_.enable_pipelined_write) {
		return PipelinedWriteImpl(write_options, my_batch, callback,
					  log_used, log_ref, disable_memtable);
	}
	...
	StopWatch write_sw(env_, immutable_db_options_.statistics.get(),
			   DB_WRITE);
	...
	if (w.state == WriteThread::STATE_PARALLEL_MEMTABLE_WRITER) {
		...
		if (w.ShouldWriteToMemtable()) {
			...
			w.status = WriteBatchInternal::InsertInto(
				&w, w.sequence, &column_family_memtables,
				&flush_scheduler_,
	...
	if (status.ok()) {
		...
		stats->AddDBStats(InternalStats::NUMBER_KEYS_WRITTEN,
				  total_count);
		RecordTick(stats_, NUMBER_KEYS_WRITTEN, total_count);
		stats->AddDBStats(InternalStats::BYTES_WRITTEN,
				  total_byte_size);
		...
		if (write_options.disableWAL) {
			has_unpersisted_data_.store(true,
		...
		if (status.ok() && !write_options.disableWAL) {
			...
			status = WriteToWAL(write_group, cur_log_writer,
		...
		if (status.ok()) {
			...
			if (!parallel) {
				...
				write_group.running.store(
					static_cast<uint32_t>(write_group.size),
					std::memory_order_relaxed);
				  ...
				  bool disable_memtable)
{
	...
	if (w.state == WriteThread::STATE_GROUP_LEADER) {
		...
		mutex_.Lock();
		bool need_log_sync =
			!write_options.disableWAL && write_options.sync;
		bool need_log_dir_sync = need_log_sync && !log_dir_synced_;
		w.status = PreprocessWrite(write_options, &need_log_sync,
					   &write_context);
		log::Writer *cur_log_writer = logs_.back().writer;
		mutex_.Unlock();
		...
		const SequenceNumber current_sequence =
			write_thread_.UpdateLastSequence(
		...
		if (w.status.ok()) {
			...
			for (auto writer : wal_write_group) {
				if (writer->CheckCallback(this)) {
					...
					total_byte_size = WriteBatchInternal::
						AppendedByteSize(
							total_byte_size,
							WriteBatchInternal::ByteSize(
								writer->batch));
			...
			if (w.disable_wal) {
				has_unpersisted_data_.store(
					true, std::memory_order_relaxed);
			}
			write_thread_.UpdateLastSequence(current_sequence +
							 total_count - 1);
		}
		...
		if (w.ShouldWriteToWAL()) {
			...
			if (wal_write_group.size > 1) {
				stats->AddDBStats(
					InternalStats::WRITE_DONE_BY_OTHER,
					wal_write_group.size - 1);
				RecordTick(stats_, WRITE_DONE_BY_OTHER,
					   wal_write_group.size - 1);
			}
			w.status = WriteToWAL(wal_write_group, cur_log_writer,
					      need_log_sync, need_log_dir_sync,
					      current_sequence);
		}
		...
		if (need_log_sync) {
			mutex_.Lock();
			MarkLogsSynced(logfile_number_, need_log_dir_sync,
				       w.status);
			mutex_.Unlock();
		}
	...
	if (w.state == WriteThread::STATE_MEMTABLE_WRITER_LEADER) {
		...
		if (memtable_write_group.size > 1 &&
			...
			memtable_write_group
				...
				memtable_write_group, w.sequence,
				column_family_memtables_.get(),
				&flush_scheduler_,
				write_options.ignore_missing_column_families,
				0 /*log_number*/, this);
			versions_->SetLastSequence(
				memtable_write_group.last_sequence);
			write_thread_.ExitAsMemTableWriter(
				&w, memtable_write_group);
		}
	}
...
void DBImpl::ParanoidCheck(const Status &status)
{
	...
	if (immutable_db_options_.paranoid_checks && !status.ok() &&
		...
		if (bg_error_.ok()) {
			bg_error_ =
				status; // stop compaction & fail any further writes
		}
		mutex_.Unlock();
	}
}
...
Status DBImpl::PreprocessWrite(const WriteOptions &write_options,
			       bool *need_log_sync, WriteContext *write_context)
{
	mutex_.AssertHeld();
			  ...
			  bool need_log_dir_sync, SequenceNumber sequence)
{
	...
	if (write_group.size == 1 && leader->ShouldWriteToWAL() &&
		...
		for (auto writer : write_group) {
			if (writer->ShouldWriteToWAL()) {
				WriteBatchInternal::Append(merged_batch,
							   writer->batch,
							   /*WAL_only*/ true);
				write_with_wal++;
			}
	...
	if (status.ok()) {
		...
		stats->AddDBStats(InternalStats::WRITE_WITH_WAL,
				  write_with_wal);
...
Status DBImpl::DelayWrite(uint64_t num_bytes, const WriteOptions &write_options)
{
	...
	if (delayed) {
		default_cf_internal_stats_->AddDBStats(
			InternalStats::WRITE_STALL_MICROS, time_delayed);
					    ...
					    WriteBatch *my_batch)
{
	...
	if (write_controller_.NeedSpeedupCompaction()) {
		if (allow_2pc() &&
		    (my_batch->HasCommit() || my_batch->HasRollback())) {
		...
		if (write_options.no_slowdown) {
			...
			write_controller_.low_pri_rate_limiter()->Request(
				my_batch->GetDataSize(), Env::IO_HIGH, nullptr);
		}
	}
	return Status::OK();
}
...
Status DBImpl::SwitchMemtable(ColumnFamilyData *cfd, WriteContext *context)
{
	...
	const auto preallocate_block_size = GetWalPreallocateBlockSize(
		mutable_cf_options.write_buffer_size);
	mutex_.Unlock();
	Status s;
	{
		if (creating_new_log) {
			EnvOptions opt_env_opt = env_->OptimizeForLogWrite(
			...
			if (s.ok()) {
				...
				unique_ptr<WritableFileWriter> file_writer(
					new WritableFileWriter(std::move(lfile),
							       opt_env_opt));
...
size_t DBImpl::GetWalPreallocateBlockSize(uint64_t write_buffer_size) const
{
	...
	if (mutable_db_options_.max_total_wal_size > 0) {
		bsize = std::min<size_t>(
			bsize, mutable_db_options_.max_total_wal_size);
	}
	if (immutable_db_options_.db_write_buffer_size > 0) {
		bsize = std::min<size_t>(
			bsize, immutable_db_options_.db_write_buffer_size);
	}
	if (immutable_db_options_.write_buffer_manager &&
	    immutable_db_options_.write_buffer_manager->enabled()) {
		bsize = std::min<size_t>(
			bsize, immutable_db_options_.write_buffer_manager
				       ->buffer_size());
	}
