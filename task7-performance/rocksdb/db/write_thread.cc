...
WriteThread::WriteThread(const ImmutableDBOptions &db_options)
	: max_yield_usec_(db_options.enable_write_thread_adaptive_yield ?
	  ...
	  newest_writer_(nullptr), newest_memtable_writer_(nullptr),
	  last_sequence_(0)
{
}
...
void WriteThread::JoinBatchGroup(Writer *w)
{
     ...
     */
		...
		TEST_SYNC_POINT_CALLBACK(
			"WriteThread::JoinBatchGroup:DoneWaiting", w);
	}
}
...
void WriteThread::EnterAsMemTableWriter(Writer *leader, WriteGroup *write_group)
{
	...
	if (!allow_concurrent_memtable_write_ || !leader->batch->HasMerge()) {
		...
		while (w != newest_writer) {
			...
			if (!allow_concurrent_memtable_write_) {
				auto batch_size =
					WriteBatchInternal::ByteSize(w->batch);
				if (size + batch_size > max_size) {
					// Do not make batch too big
					break;
				}
	...
	if (enable_pipelined_write_) {
		...
		if (write_group.size > 0) {
			if (LinkGroup(write_group, &newest_memtable_writer_)) {
				...
				SetState(write_group.leader,
					 STATE_MEMTABLE_WRITER_LEADER);
			}
		}
		// Reset newest_writer_ and wake up the next leader.
		Writer *newest_writer = last_writer;
		if (!newest_writer_.compare_exchange_strong(newest_writer,
							    nullptr)) {
		...
		if (head != last_writer ||
			...
			assert(last_writer->link_newer->link_older ==
			       last_writer);
