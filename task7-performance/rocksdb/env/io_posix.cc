...
size_t GetLogicalBufferSize(int __attribute__((__unused__)) fd)
{
	...
	if (device_dir.substr(parent_begin + 1,
			      parent_end - parent_begin - 1) != "block") {
...
PosixSequentialFile::PosixSequentialFile(const std::string &fname, FILE *file,
	...
	: filename_(fname), file_(file), fd_(fd),
	...
	*result = Slice(scratch, r);
	if (r < n) {
		if (feof(file_)) {
			...
			s = IOError("While reading file sequentially",
				    filename_, errno);
		}
	}
					   ...
					   Slice *result, char *scratch)
{
	...
	while (left > 0) {
		...
		offset += r;
		left -= r;
		if (r % static_cast<ssize_t>(GetRequiredBufferAlignment()) !=
		    0) {
	...
	if (r < 0) {
		...
		s = IOError("While pread " + ToString(n) +
...
Status PosixSequentialFile::Skip(uint64_t n)
{
	if (fseek(file_, static_cast<long int>(n), SEEK_CUR)) {
		return IOError("While fseek to skip " + ToString(n) + " bytes",
			       filename_, errno);
	}
	return Status::OK();
}
...
size_t PosixHelper::GetUniqueIdFromFile(int fd, char *id, size_t max_size)
{
	...
	TEST_SYNC_POINT_CALLBACK("GetUniqueIdFromFile:FS_IOC_GETVERSION",
				 &result);
	if (result == -1) {
		return 0;
	}
				   ...
				   char *scratch) const
{
	...
	while (left > 0) {
		...
		left -= r;
		if (use_direct_io() &&
		    r % static_cast<ssize_t>(GetRequiredBufferAlignment()) !=
			    0) {
			// Bytes reads don't fill sectors. Should only happen at the end
			// of the file.
			break;
	...
	if (r < 0) {
		// An error: return a non-ok status
		s = IOError("While pread offset " + ToString(offset) + " len " +
				    ToString(n),
			    filename_, errno);
	}
	*result = Slice(scratch, (r < 0) ? 0 : n - left);
...
Status PosixMmapFile::UnmapCurrentRegion()
{
	...
	if (base_ != nullptr) {
		...
		if (munmap_status != 0) {
			return IOError("While munmap", filename_,
				       munmap_status);
		}
...
Status PosixMmapFile::MapNewRegion()
{
	...
	if (allow_fallocate_) {
		...
		if (alloc_status != 0) {
			...
			alloc_status =
				posix_fallocate(fd_, file_offset_, map_size_);
		}
		if (alloc_status != 0) {
			return Status::IOError(
				"Error allocating space to file : " +
				filename_ +
...
PosixMmapFile::PosixMmapFile(const std::string &fname, int fd, size_t page_size,
			     const EnvOptions &options)
	: filename_(fname), fd_(fd), page_size_(page_size),
	  map_size_(Roundup(65536, page_size)), base_(nullptr), limit_(nullptr),
	  dst_(nullptr), last_sync_(nullptr), file_offset_(0)
{
#ifdef ROCKSDB_FALLOCATE_PRESENT
...
Status PosixMmapFile::Allocate(uint64_t offset, uint64_t len)
{
	...
	if (allow_fallocate_) {
		alloc_status = fallocate(
			fd_,
			fallocate_with_keep_size_ ? FALLOC_FL_KEEP_SIZE : 0,
			static_cast<off_t>(offset), static_cast<off_t>(len));
	}
	if (alloc_status == 0) {
		return Status::OK();
	} else {
		return IOError("While fallocate offset " + ToString(offset) +
				       " len " + ToString(len),
			       filename_, errno);
	}
}
...
PosixWritableFile::PosixWritableFile(const std::string &fname, int fd,
				     const EnvOptions &options)
	: filename_(fname), use_direct_io_(options.use_direct_writes), fd_(fd),
	  filesize_(0), logical_sector_size_(GetLogicalBufferSize(fd_))
{
#ifdef ROCKSDB_FALLOCATE_PRESENT
	allow_fallocate_ = options.allow_fallocate;
	fallocate_with_keep_size_ = options.fallocate_with_keep_size;
...
Status PosixWritableFile::Append(const Slice &data)
{
	if (use_direct_io()) {
		...
		assert(IsSectorAligned(data.data(),
				       GetRequiredBufferAlignment()));
	}
	const char *src = data.data();
	size_t left = data.size();
...
Status PosixWritableFile::Truncate(uint64_t size)
{
	...
	if (r < 0) {
		s = IOError("While ftruncate file to size " + ToString(size),
			    filename_, errno);
	} else {
		filesize_ = size;
	}
...
Status PosixWritableFile::Close()
{
		...
		if (result == 0 &&
			...
			if (allow_fallocate_) {
				fallocate(fd_,
					  ...
					  filesize_,
					  block_size * last_allocated_block -
						  filesize_);
			}
		}
#endif
	...
	if (close(fd_) < 0) {
		s = IOError("While closing file after writing", filename_,
			    errno);
	}
	fd_ = -1;
	return s;
...
Status PosixWritableFile::RangeSync(uint64_t offset, uint64_t nbytes)
{
	...
	if (sync_file_range(fd_, static_cast<off_t>(offset),
		...
		return IOError("While sync_file_range offset " +
				       ToString(offset) + " bytes " +
				       ToString(nbytes),
			       filename_, errno);
	}
}
...
Status PosixRandomRWFile::Write(uint64_t offset, const Slice &data)
{
	...
	while (left != 0) {
		...
		if (done < 0) {
			...
			return IOError(
				"While write random read/write file at offset " +
					ToString(offset),
				filename_, errno);
		}
			       ...
			       char *scratch) const
{
	...
	while (left > 0) {
		...
		if (done < 0) {
			...
			return IOError(
				"While reading random read/write file offset " +
					ToString(offset) + " len " +
					ToString(n),
				filename_, errno);
		} else if (done == 0) {
...
Status PosixRandomRWFile::Flush()
{
	return Status::OK();
}
...
{
	if (fdatasync(fd_) < 0) {
		return IOError("While fdatasync random read/write file",
			       filename_, errno);
	}
	return Status::OK();
}
