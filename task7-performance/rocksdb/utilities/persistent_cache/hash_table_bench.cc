...
int main()
{
	fprintf(stderr, "Please install gflags to run tools\n");
}
#else
...
template <class Key, class Value> class HashTableImpl {
    public:
...
class HashTableBenchmark {
	...
	explicit HashTableBenchmark(HashTableImpl<size_t, std::string> *impl,
		...
		: impl_(impl), sec_(sec), ninserts_(0), nreads_(0), nerases_(0),
		  nerases_failed_(0), quit_(false)
	{
		Prepop();
		...
		if (sec_) {
			...
			printf("erase fail = %d (%f%%)\n",
			       ...
			       static_cast<float>(nerases_failed_ / nerases_ *
						  100));
			printf("====== \n");
		}
	}
	...
	void Prepop()
	{
		for (size_t i = 0; i < max_prepop_key; ++i) {
			bool status =
				impl_->Insert(i, std::string(1000, i % 255));
			assert(status);
		}
		...
		for (size_t i = 0; i < 10 * max_prepop_key; ++i) {
			bool status = impl_->Insert(insert_key_++,
						    std::string(1000, 'x'));
			assert(status);
		}
	}
...
class GranularLockImpl : public HashTableImpl<size_t, string> {
    ...
    private:
	struct Node {
		explicit Node(const size_t key, const string &val)
			: key_(key), val_(val)
		{
		}
...
int main(int argc, char **argv)
{
	...
	printf("Micro benchmarking std::unordered_map \n");
	{
		...
		rocksdb::HashTableBenchmark _(&impl, FLAGS_nsec,
					      FLAGS_nthread_write,
					      FLAGS_nthread_read,
					      FLAGS_nthread_erase);
	}
