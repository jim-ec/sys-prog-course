...
class WBWIIteratorImpl : public WBWIIterator {
	...
	WBWIIteratorImpl(uint32_t column_family_id,
		...
		: column_family_id_(column_family_id),
		  skip_list_iter_(skip_list), write_batch_(write_batch)
	{
	}
	...
	virtual void SeekToFirst() override
	{
		WriteBatchIndexEntry search_entry(
			WriteBatchIndexEntry::kFlagMin, column_family_id_, 0,
			0);
	...
	{
		WriteBatchIndexEntry search_entry(
			WriteBatchIndexEntry::kFlagMin, column_family_id_ + 1,
			0, 0);
		skip_list_iter_.Seek(&search_entry);
		if (!skip_list_iter_.Valid()) {
	...
	virtual WriteEntry Entry() const override
	{
		...
		auto s = write_batch_->GetEntryFromDataOffset(
			iter_entry->offset, &ret.type, &ret.key, &ret.value,
			&blob, &xid);
		assert(s.ok());
		assert(ret.type == kPutRecord || ret.type == kDeleteRecord ||
		       ret.type == kSingleDeleteRecord ||
		       ret.type == kDeleteRangeRecord ||
...
struct WriteBatchWithIndex::Rep {
	...
	bool UpdateExistingEntry(ColumnFamilyHandle *column_family,
				 const Slice &key);
	bool UpdateExistingEntryWithCfId(uint32_t column_family_id,
					 const Slice &key);
	...
	uint32_t column_family_id, const Slice &key)
{
	...
	if (comparator.CompareKey(column_family_id, key, iter.Entry().key) !=
	    0) {
		return false;
	}
...
void WriteBatchWithIndex::Rep::AddNewEntry(uint32_t column_family_id)
{
	...
	// Extract key
	Slice key;
	bool success __attribute__((__unused__)) = ReadKeyFromWriteBatchEntry(
		&entry_ptr, &key, column_family_id != 0);
	assert(success);

	auto *mem = arena.Allocate(sizeof(WriteBatchIndexEntry));
	auto *index_entry = new (mem)
		WriteBatchIndexEntry(last_entry_offset, column_family_id,
...
Status WriteBatchWithIndex::Rep::ReBuildIndex()
{
	...
	while (s.ok() && !input.empty()) {
		...
		s = ReadRecordFromWriteBatch(&input, &tag, &column_family_id,
					     &key, &value, &blob, &xid);
		if (!s.ok()) {
			break;
		}
		...
		case kTypeMerge:
			found++;
			if (!UpdateExistingEntryWithCfId(column_family_id,
							 key)) {
					 ...
					 const Slice &key, std::string *value)
{
	...
	WriteBatchWithIndexInternal::Result result =
		WriteBatchWithIndexInternal::GetFromBatch(
			immuable_db_options, this, column_family, key,
			&merge_context, &rep->comparator, value,
			rep->overwrite_key, &s);
...
Status WriteBatchWithIndex::GetFromBatchAndDB(DB *db,
					      ...
					      std::string *value)
{
	return GetFromBatchAndDB(db, read_options, db->DefaultColumnFamily(),
				 key, value);
}
					      ...
					      std::string *value)
{
	...
	WriteBatchWithIndexInternal::Result result =
		WriteBatchWithIndexInternal::GetFromBatch(
			immuable_db_options, this, column_family, key,
			&merge_context, &rep->comparator, &batch_value,
			rep->overwrite_key, &s);
	...
	if (s.ok() || s.IsNotFound()) { // DB Get Succeeded
		if (result ==
			...
			Statistics *statistics =
				immuable_db_options.statistics.get();
